...
public class WheelScreen extends Screen {
    ...
    private static void tri(GuiGraphics g, float x1, float y1, float x2, float y2, float x3, float y3, int argb) {
        ...
        buf.vertex(mat, x1, y1, 0).color(r, gr, b, al).endVertex();
        buf.vertex(mat, x2, y2, 0).color(r, gr, b, al).endVertex();
        buf.vertex(mat, x3, y3, 0).color(r, gr, b, al).endVertex();
    ...
    private static void arc(GuiGraphics g, float cx, float cy, float r1, float r2,
        ...
        for (int i = 0; i <= steps; i++) {
            ...
            float cos = Mth.cos(a);
            float sin = Mth.sin(a);
            buf.vertex(mat, cx + cos * r2, cy + sin * r2, 0).color(or, og, ob, oa).endVertex();
            buf.vertex(mat, cx + cos * r1, cy + sin * r1, 0).color(ir, ig, ib, ia).endVertex();
        }
        BufferUploader.drawWithShader(buf.end());
        RenderSystem.disableBlend();
    }
