...
public class VillagerBrowserScreen extends Screen implements TextTyping {
    ...
    private static String stackKey(ItemStack s) {
        ...
        String tag = s.getTag() == null ? "" : s.getTag().toString();
        return id + "#" + tag;
    }
    ...
    private static ItemEntry makeEntry(ItemStack src) {
        ...
        icon.setCount(1);
        ResourceLocation id = ForgeRegistries.ITEMS.getKey(icon.getItem());
        String idStr = id == null ? "?" : id.toString();
        String name = icon.getHoverName().getString();
        StringBuilder blob = new StringBuilder();
        blob.append(name.toLowerCase(Locale.ROOT)).append(' ').append(idStr).append(' ');
        String suffix = "";
        Map<Enchantment, Integer> ench = EnchantmentHelper.getEnchantments(icon);
        if (!ench.isEmpty()) {
            StringBuilder names = new StringBuilder();
            for (Map.Entry<Enchantment, Integer> en : ench.entrySet()) {
                ResourceLocation ek = ForgeRegistries.ENCHANTMENTS.getKey(en.getKey());
                String eName = I18n.get(en.getKey().getDescriptionId());
                String lvl = I18n.exists("enchantment.level." + en.getValue())
                        ? I18n.get("enchantment.level." + en.getValue()) : String.valueOf(en.getValue());
                if (names.length() > 0) {
                    names.append(", ");
                }
        ...
        if (icon.getTag() != null) {
            blob.append(icon.getTag().toString().toLowerCase(Locale.ROOT)).append(' ');
        }
        String ns = id == null ? "" : id.getNamespace();
        String modName = ModList.get().getModContainerById(ns)
    ...
    private void drawPreview(GuiGraphics g, Prof p, int cx, int cy) {
        ...
        try {
            InventoryScreen.renderEntityInInventory(g, cx, cy, 28, pose, null, ent);
    ...
    private String dumpProf(Prof p) {
        ...
        if (!p.workstations().isEmpty()) {
            ...
            for (int i = 0; i < p.workstations().size(); i++) {
                ...
                sb.append(ForgeRegistries.ITEMS.getKey(p.workstations().get(i).getItem()));
            }
            sb.append('\n');
        }
        int lastLevel = Integer.MIN_VALUE;
    ...
    private static String dumpStack(ItemStack s) {
        ...
        return s.getCount() + "x " + ForgeRegistries.ITEMS.getKey(s.getItem());
    }
        ...
        if (button == 0 && mode == 0 && selected != null
            ...
            if (this.minecraft != null && this.minecraft.getSingleplayerServer() != null) {
                ...
                String id = selected.wandering() || selected.profession() == null ? ""
                        : String.valueOf(ForgeRegistries.VILLAGER_PROFESSIONS.getKey(selected.profession()));
