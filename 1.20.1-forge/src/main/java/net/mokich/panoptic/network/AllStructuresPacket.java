...
public class AllStructuresPacket {
    ...
    public static void handle(AllStructuresPacket msg, Supplier<Context> ctx) {
        ctx.get().enqueueWork(() -> {
            ...
            if (!list.isEmpty()) {
                Panoptic.CHANNEL.send(PacketDistributor.PLAYER.with(() -> player),
                        new AllStructuresResultPacket(list));
            }
        });
